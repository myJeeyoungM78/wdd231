...
const url = "data/diylist.json";

async function getMeterialData() {

    const response = await fetch(url);
    const data = await response.json();

    // console.table(data.members);
    displayMeterials(data.meterials);
    // displayListMembers(data.members);
}

getMeterialData();

const displayMeterials = (meterials) => {
    meterials.forEach((meterial) => {
        let card = document.createElement('article');        

        let meterial = document.createElement('h3');
        // let industry = document.createElement('p');
        let diy = document.createElement('img');
        ...
        meterial.textContent = `${meterial.meterial}`;
        // industry.textContent = `${member.industry}`;
        // address.textContent = ``;
        // email.innerHTML = `<strong>Email</strong>: ${member.email}`;
        // phone.innerHTML = `<strong>Phone</strong>: ${member.phone}`;
        link.innerHTML = `<strong>Url</strong>: ${meterial.link}`;
        diy.setAttribute('src', meterial.diy);
        diy.setAttribute('alt', `diy of ${meterial.meterial}`);
        diy.setAttribute('loading', 'lazy');
        diy.setAttribute('width', '340');
        diy.setAttribute('height', '440');

        card.appendChild(meterial);
        // card.appendChild(industry)
        card.appendChild(diy);
